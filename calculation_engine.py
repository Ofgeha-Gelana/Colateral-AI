...
# ... (All previous helper functions remain here, unchanged) ...
...
def calculate_under_construction_value(full_value: float, building_type: str, grade: str, incomplete_components: list) -> tuple[float, float]:
...
def run_full_valuation(valuation_data: dict) -> dict:
    ...
    for i, building in enumerate(valuation_data.get('buildings', [])):
        ...
        if category in ["Higher Villa", "Multi-Story Building", "MPH & Factory Building", "Apartment / Condominium"]:
            ...
            if category == "Higher Villa": building_type_for_rate = "Single Story Building (higher Villa)"; policy_check_type = "Higher Villa"
            elif 1 <= num_floors <= 3: building_type_for_rate = "G+1 and G+2"; policy_check_type = "G+1-3"
            elif num_floors >= 4: building_type_for_rate = "G+3 and G+4"; policy_check_type = "G+4 & above"
            else: building_type_for_rate = "Single Story Building (higher Villa)"; policy_check_type = "Higher Villa"
            ...
            if building.get('is_under_construction', False):
                building_cost, completed_percent = calculate_under_construction_value(full_replacement_cost, building_type_for_rate, grade, building.get('incomplete_components', []))
                min_completion = minimum_completion_stages.get(policy_check_type, 0)
                if completed_percent < min_completion:
                    validation_warnings.append(f"Warning: Building '{building.get('name')}' is only {completed_percent:.0%} complete, which is below the required minimum of {min_completion:.0%} for a loan.")
            else:
    ...
    if valuation_data['buildings'][0]['category'] == "Apartment / Condominium":
        ...
        if grade in ["Excellent", "Good"]: plot_area_factor = 0.8
        else: plot_area_factor = 0.4
        plot_area *= plot_area_factor
        
    calculated_lv = calculate_location_value(property_details.get('prop_town', ''), property_details.get('gen_use', ''), property_details.get('plot_grade', ''), plot_area)
